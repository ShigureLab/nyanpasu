...
状态：实现与评估方案，2026-09-24。通用子任务、根并发额度、级联回收、独立 base 快照、reviewer prompts、证据冻结和 Dashboard 已实现为四层 PR stack。本文保留完整设计依据；当前接口见下一节。尚未执行历史 PR 的模型盲评，也没有执行全仓测试质量审计。
...
这些是语义策略，写在 reviewer 指令中；不写成按代码行数或后缀分流的 Python 分类器。系统检查显式要求是否完成、产物是否存在、版本是否匹配；不假装用 schema 检查能够判断跳过理由是否合理。
...
等待子任务时调用持久化 await 接口并按返回指令结束当前 turn。
子任务共享本轮根任务的并发额度；结束 turn 用于等待和恢复，不是为了给子任务腾额度。
子任务失败、取消或产物过期时如实处理，不用无发现替代未完成。
